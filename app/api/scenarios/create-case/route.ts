...
export async function POST(req: NextRequest) {
  try {
    const { scenarioId, userId, role } = await req.json()
    if (!scenarioId || !userId || !role) return NextResponse.json({ error: 'Missing fields' }, { status: 400 })
    ...
    const { data: c, error: caseErr } = await supabase
      .from('cases')
      .insert({ title: scen.title, summary: scen.facts || scen.summary, case_type: caseType, created_by: userId })
      .select('id')
      .single()
    if (caseErr) return NextResponse.json({ error: caseErr.message }, { status: 400 })

    const { error: partErr } = await supabase.from('case_participants').insert({ case_id: c.id, user_id: userId, role })
